...
- ESLint 규칙이 React 코드에서 정상적으로 동작하는가
- TypeScript 설정이 React + Hooks 환경에서 문제없이 적용되는가
- Prettier와 ESLint 간 충돌 없이 포맷팅이 이루어지는가
- Vite 기반 React 환경에서 설정이 깨지지 않는가
...
```text
@ddoblue/eslint-config/react-strict
@ddoblue/tsconfig/react.json
